...
inline real_t imag(const real_t)
{
  return 0.;
}
#endif // HDRderivify
...
public:
  ...
  cplx(const real_t &d)
      : complex<real_t>(d){};
  cplx(const real_t &r, const real_t &i)
      : complex<real_t>(r, i){};
  cplx(const complex<double> &z)
      : complex<real_t>(z){};
  cplx(const complex<float> &z)
      : complex<real_t>(z){};
  operator real_t()
  {
    return this->real();
...
inline multicomplex init_from_C1(complex_t C)
{
  multicomplex out;
